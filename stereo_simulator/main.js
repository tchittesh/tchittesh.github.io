...
class ConvexPolyhedron {
  ...
  toMesh(material) {
    ...
    const intersectThreePlanes = (plane1, plane2, plane3) => {
      ...
      const detX = d1 * (n2.y * n3.z - n3.y * n2.z) -
                   n1.y * (d2 * n3.z - d3 * n2.z) +
                   n1.z * (d2 * n3.y - d3 * n2.y);
                   
      const detY = n1.x * (d2 * n3.z - d3 * n2.z) -
                   d1 * (n2.x * n3.z - n3.x * n2.z) +
                   n1.z * (n2.x * d3 - n3.x * d2);
                   
      const detZ = n1.x * (n2.y * d3 - n3.y * d2) -
                   n1.y * (n2.x * d3 - n3.x * d2) +
                   d1 * (n2.x * n3.y - n3.x * n2.y);
...
function updateGUI(camera) {
  ...
  if (camera.isStereo) {
    titleElement.style.background = `linear-gradient(90deg, rgb(${camera.leftColor.r * 255}, ${camera.leftColor.g * 255}, ${camera.leftColor.b * 255}), rgb(${camera.rightColor.r * 255}, ${camera.rightColor.g * 255}, ${camera.rightColor.b * 255}))`;
  } else {
    titleElement.style.background = `rgb(${camera.color.r * 255}, ${camera.color.g * 255}, ${camera.color.b * 255})`;
  }

  folder.add(camera.camera.position, 'x', -100, 100).name('Position X').onChange(() => updateCamera(camera));
  folder.add(camera.camera.position, 'y', -100, 100).name('Position Y').onChange(() => updateCamera(camera));
  folder.add(camera.camera.position, 'z', -100, 100).name('Position Z').onChange(() => updateCamera(camera));
  folder.add(camera.camera.rotation, 'x', -Math.PI, Math.PI).name('Rotation X').onChange(() => updateCamera(camera));
  folder.add(camera.camera.rotation, 'y', -Math.PI, Math.PI).name('Rotation Y').onChange(() => updateCamera(camera));
  folder.add(camera.camera.rotation, 'z', -Math.PI, Math.PI).name('Rotation Z').onChange(() => updateCamera(camera));
  folder.add(camera.camera, 'fov', 10, 120).name('Field of View').onChange(() => updateCamera(camera));
  folder.add(camera, 'width', 240, 3840).step(1).name('Width').onChange(() => updateCamera(camera));
  folder.add(camera, 'height', 135, 2160).step(1).name('Height').onChange(() => updateCamera(camera));
  if (camera.isStereo) {
    folder.add(camera.stereoCamera, 'eyeSep', 0.1, 10).name('Baseline').onChange(() => updateCamera(camera));
  }
...
function updateProjections() {
  ...
  for (let i = 0; i < positionAttribute.count; i++) {
    ...
    for (const camera of allCameras) {
      ...
      const rayPolyhedron = createSquarePyramidForPixelInCamera(
        camera.camera,
        proj.pixelCoords.x,
        proj.pixelCoords.y,
        camera.width,
...
function projectPointToUV(point, camera, width, height) {
  ...
  const isVisible = (
    projectedPoint.z >= -1 && projectedPoint.z <= 1 &&  // In front of camera, between near and far planes
    projectedPoint.x >= -1 && projectedPoint.x <= 1 &&  // Within horizontal view
    projectedPoint.y >= -1 && projectedPoint.y <= 1 &&  // Within vertical view
    pixelCoords &&
    pixelCoords.x >= 0 &&
    pixelCoords.y >= 0 &&
    pixelCoords.x < width &&
...
function createSquarePyramidForPixelInCamera(camera, pixelX, pixelY, width, height) {
  ...
  const unprojectNDC = (ndcPoint) => {
    ...
    const viewCoord = new THREE.Vector3(
...
const object = intersects[0].object;
    ...
    infoDisplay.innerHTML = `Triangulation Dimensions:<br>X: ${bounds.max.x - bounds.min.x}<br>Y: ${bounds.max.y - bounds.min.y}<br>Z: ${bounds.max.z - bounds.min.z}`;
